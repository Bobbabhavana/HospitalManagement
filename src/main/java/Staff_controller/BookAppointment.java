...
public class BookAppointment extends HttpServlet{
	 protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException{
		 ...
		 List<Appointment> list1=patient.getAppointements();
			if(list1==null)
			{
				list1=new ArrayList<Appointment>();
			}
			list1.add(appointment);
			patient.setAppointements(list1);
			
			List<Appointment> list2=doctor.getAppointements();
			if(list2==null)
			{
				list2=new ArrayList<Appointment>();
			}
			list2.add(appointment);
			doctor.setAppointements(list2);
		
	 dao.saveAppointment(appointment);
	 dao.updateDoctor(doctor);
	 dao.updatePatient(patient);
	 
	 resp.getWriter().print("<h1>Appointment of " +patient.getName()+" is bookeed with " +doctor.getName()+" for "+problem+"</h1>");
